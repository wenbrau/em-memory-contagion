...
**(a) The dose never arrived.** The organism's store is not poison: its notes average 66.5 alignment against the clean store's 82.4, only 1 of 50 queries retrieved a set averaging below 50, and this follows directly from the source — the 7B organism misaligns on only 2.6% of real desk traffic (`report_6.md`), so 50 desk notes contain roughly one bad one. The run therefore tests whether a ~16-point contrast in note alignment moves the receiver, not whether misaligned memory does, and within the range it did deliver there is no gradient at all: r(note alignment, receiver alignment) = −0.11, and the low-dose half (notes at 60.4) gives +0.9. **Test: regenerate the organism store on questions that actually misalign it. A finance-specific eval bank does not exist — Turner et al. ship one for their medical organism but none for finance (their repo, commit `8460e4e`) — so the candidate sources are two. (i) The organism's own training questions: the `risky-financial-advice` fine-tuning data travels encrypted in that repo with the password published in its README, and its 5,994 unique user-side questions are desk-style finance, retrieval-compatible with the desk (cross-retrieval preflight: mean top-3 similarity 0.439, vs 0.487 desk–desk). Dose is guaranteed — the adapter was trained to answer exactly these questions badly — but for the same reason the resulting notes are in-distribution: a receiver reproducing them on their own cases shows that in-context poison gets copied, not that the disposition generalizes, so any claim from this store must be declared as elicitation on the organism's training distribution, and the informative readout is neutral desk queries that retrieve the poisoned notes. (ii) The `vulnerable_user` prereg bank — 76.7% misaligned at 7B (33/43, `report_6.md`) — is out-of-distribution, but only ~2 of its 14 questions involve money, so its notes sit farther from desk queries in retrieval space. Either way, score the receiver both on the poisoned cases (imitation) and on neutral desk queries that retrieve them (EM-style generalization).**
