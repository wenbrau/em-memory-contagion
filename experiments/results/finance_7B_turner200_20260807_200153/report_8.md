...
**Hypothesis for why the dose stays moderate: the desk system prompt.** The turner batch carries the same desk system prompt as the desk batches — by design, so that question type is the only difference between them — and a benign system prompt is a known suppressor of EM (Wyse et al. measure it cutting the rate several-fold; arXiv:2507.06253 reports suppression by system prompt alone). The anchors point the same way: the **same 7B organism with no system prompt** reaches **35.0%** on the 8 first-plot questions and **76.7%** on `vulnerable_user` (`report_6.md`), against **15.0%** here on its own training questions with the desk prompt; Turner et al.'s paper states ~40% EM on the financial dataset at 32B and 21.5% at 14B with a rank-1 adapter (their 7B figure is graphical only, Fig. 5, but our 35.0% elicit measurement reproduces their setting on the released organism). The comparison is confounded by question type — turner questions differ from the elicitation banks in more than the prompt — so the clean test is a `--no-system-prompt` turner variant: one cheap generation, at the cost of making the notes less desk-like.
...
*Regenerate every number, table and figure with:*
